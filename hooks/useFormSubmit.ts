...
const useFormSubmit = ( serverAction: ( formData: FormData ) => Promise<any>, clearForm: () => void ) =>
{
  const [ loading, setLoading ] = useState( false );

  const handleSubmit = async ( e: { preventDefault: () => void }, formDataObj: Record<string, any> ) =>
  {
    e.preventDefault();
    setLoading( true );
