...
import { pgTable, uuid, text, timestamp, numeric, serial, varchar, integer, boolean, PgColumn, PgTableWithColumns, date, jsonb } from 'drizzle-orm/pg-core';
...
export const orgCustomers = pgTable( 'org_customers', {
    ...
    metadata: jsonb( 'metadata' ),
    notes: text( 'notes' ),
    favoriteEventId: uuid( 'favorite_event_id' )
        .references( () => events.id ), // Reference to favorite event
    favoritePerformerId: uuid( 'favorite_performer_id' )
        .references( () => orgPerformers.id ), // Reference to favorite performer
    createdAt: timestamp( 'created_at' ).default( sql`now()` ),
    updatedAt: timestamp( 'updated_at' ).default( sql`now()` ),
} );
...
export const visitorSchedules = pgTable( 'visitor_schedules', {
    ...
    eventId: uuid( 'event_id' )
        ...
        .references( () => events.id ), // Reference to the event
    sessionId: uuid( 'session_id' )
        .references( () => eventSessions.id ), // Reference to the selected session
    createdAt: timestamp( 'created_at' ).default( sql`now()` ),
    updatedAt: timestamp( 'updated_at' ).default( sql`now()` )
} );
...
export const orgTransactions = pgTable( 'org_transactions', {
    ...
    applicationFeeAmount: numeric( 'application_fee_amount', { precision: 10, scale: 2 } ).notNull().default( sql`0.50` ), // application fee amount
...
export const emailCampaigns = pgTable( 'email_campaigns', {
    ...
    segmentId: uuid( 'segment_id' )
        .references( () => audienceSegments.id ), // Reference to the audience segment
