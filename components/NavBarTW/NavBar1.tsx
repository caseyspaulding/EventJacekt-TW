...
export default function NavBar1 ()
{
  ...
  const { user, signOut } = useUser(); // Use useUser hook to get user context
  ...
  return (
     <div className={`sticky top-0 z-50 shadow-sm ${isSticky ? 'bg-white ' : 'bg-white/60 backdrop-blur-none'}`}>
      <header>
                        ...
                        <Menu.Item key={item.name}>
                          {({ active }) =>
                            item.name === 'Sign out' ? (
                              <button
                                onClick={signOut}
                                className={`${active ? 'bg-gray-100' : ''} block w-full px-4 py-2 text-sm text-gray-700`}
                              >
                                {item.name}
                              </button>
                            ) : (
