...
export default function FormResponsesPage ()
{
  ...
  useEffect( () =>
  {
    async function fetchData ()
    {
      ...
      try
      {
        ...
        // Fetch form fields
        const fieldsData = await getFormFields( formId as string );
  ...
  {
    // Define CSV headers
    const headers = [ 'Response ID', 'Submitted At', ...fields.map( ( field ) => field.fieldName ) ];
  ...
  return (
    <div className="sm:px-6 p-6 rounded-2xl bg-white">
      ...
      {/* Export Button */ }
      <button className="bg-blue-700 text-white px-4 py-2 rounded-md mb-4" onClick={ handleExportToCSV }>
        Export to CSV
      </button>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-300">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-3 text-left text-sm font-semibold text-gray-900">Response ID</th>
            ...
            { filteredResponses.length > 0 ? (
              filteredResponses.map( ( response ) => (
                <tr key={ response.responseId }>
                  <td className="px-3 py-2">{ response.responseId }</td>
                  ...
                  <td className="px-3 py-2">
                    <ul className="list-disc list-inside">
                      { Object.entries( response.responseData ).map( ( [ key, value ] ) => (
                        <li key={ key }>
                          <strong>{ fieldIdToNameMap[ key ] || key }:</strong> { value }
                        </li>
                      ) ) }
                    </ul>
                  </td>
