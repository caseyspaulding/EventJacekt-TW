...
export default function StripeConnectOnboarding ( { orgId }: { orgId: string } )
{
  ...
  return (
    <div className="space-y-4">
      { !connectedAccountId && <h2 className="text-2xl font-bold">Connect your Stripe account</h2> }
      { connectedAccountId && !stripeConnectInstance && <p>Setting up your Stripe account...</p> }
      ...
      { ( connectedAccountId || accountCreatePending || onboardingExited ) && (
        <div>
          { connectedAccountId && <p>Your Stripe account is connected and ready to use.</p> }
          { accountCreatePending && <p>Creating your Stripe account...</p> }
          { onboardingExited && <p>Stripe Connect onboarding completed. You can now start selling tickets!</p> }
        </div>
      ) }
    </div>
  );
