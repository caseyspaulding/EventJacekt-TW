...
export default function BankingPageClient ( { params, isStripeConnected, initialAccountId }: BankingPageProps )
{
  const [ accountCreatePending, setAccountCreatePending ] = useState( false );
  const [ onboardingExited, setOnboardingExited ] = useState( false );
  const [ error, setError ] = useState( false );
  const [ connectedAccountId, setConnectedAccountId ] = useState<string | undefined>( initialAccountId || undefined );
  const stripeConnectInstance = useStripeConnect( connectedAccountId );
  const [ stripeAccountDetails, setStripeAccountDetails ] = useState<StripeAccountDetails | null>( null );
